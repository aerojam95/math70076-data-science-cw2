...
- One element of this algorithm that has not be made flexible to change in the ```main.py``` programme is the distance used to calcualte the $k$-nearest neighbours; the distance has been set as default to the Eculidean distance for this study; this distance calculation can be changed but requires development or modification to the ```knn.py``` module which end users are free to do  
